  ...
  module Calculator::Shipping
    class Calculator::CorreiosBaseCalculator < Spree::ShippingCalculator
      ...
      def compute_package(object)
        ...
        zipcode_stock_location = object.stock_location.zipcode

        order      = object.is_a?(Spree::Order) ? object : object.order
        package    = package_from_order(order)
        calculator = calculator_for_package_of_order(package, order, zipcode_stock_location)
      ...
      def calculator_for_package_of_order(package, order, zipcode_origin = nil)
        ::Correios::Frete::Calculador.new do |c|
          c.encomenda         = package

          c.cep_origem        = zipcode_origin || preferred_zipcode
